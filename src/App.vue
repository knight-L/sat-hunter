<template>
    <div class="flex h-screen w-screen overflow-hidden font-sans text-gray-700 bg-gray-100">
        <aside class="flex flex-col w-96 bg-white shadow-2xl z-1000 overflow-y-auto border-r border-gray-200">
            <div class="p-5 border-b border-gray-100">
                <h2 class="text-xl font-bold text-gray-800 flex items-center gap-2">
                    <svg width="40" height="40" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <circle cx="50" cy="50" r="35" stroke="#3B82F6" stroke-width="2" stroke-dasharray="4 4" />
                        <circle cx="50" cy="50" r="22" fill="#DBEAFE" />
                        <path d="M50 35V65M50 65L40 55M50 65L60 55" stroke="#1D4ED8" stroke-width="5"
            ...
            <div class="flex-1 p-5 flex flex-col gap-6">
                <div class="text-xs font-bold text-center py-2 px-3 rounded-md border transition-colors duration-300"
                    :class="[
                        currentMode === 'polygon'
                ...
                <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-4 hover:shadow-md transition-shadow">
                    ...
                    <div class="flex gap-2">
                        <input type="text" v-model="searchQuery" placeholder="输入区域名称或关键字"
                        ...
                        <button
                            ...
                            @click="searchLocation" :disabled="isSearching">
                            {{ isSearching ? "..." : "🔍" }}
                        </button>
                    </div>
                ...
                <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
                    ...
                    <input type="range" min="6" max="18" v-model.number="zoomLevel"
            ...
            <div class="p-5 border-t border-gray-100 bg-gray-50 mt-auto">
                <button
                    ...
                    :disabled="!currentMode || isDownloading" @click="startDownload">
                    {{ isDownloading ? `处理中 ${downloadProgress}%` : "开始下载" }}
                </button>

                <div v-if="isDownloading" class="mt-3 h-2 w-full bg-gray-200 rounded-full overflow-hidden">
                    <div class="h-full bg-green-500 transition-all duration-300 ease-out"
        ...
        <div id="map" ref="mapContainer" class="flex-1 h-full z-0 bg-gray-900"></div>
    </div>
</template>
...
const selectRegion = (region: RegionItem) => {
...
const searchLocation = async () => {
    ...
    try {
        ...
        const regionExactByCode = allRegions.value.find((r) => String(r.adcode) === q);
        ...
        const endpoint = `https://geo.datav.aliyun.com/areas_v3/bound/${regionCandidate.adcode}.json`;
        ...
        if (map.value && cityGeoJSON.value) {
            ...
            setMode("polygon");
            statusText.value = `已选中: ${regionCandidate.name}`;
        }
    } catch (e) {
