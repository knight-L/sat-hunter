<template>
    <div class="relative w-full" ref="containerRef">
        <div class="relative flex items-center w-full bg-white border rounded-xl cursor-text transition-all duration-200"
            :class="[
                isOpen
            ...
            <input ref="inputRef" type="text" :value="displayValue" :placeholder="selectedLabel ? '' : placeholder"
                :disabled="disabled"
            ...
            <span class="absolute right-3 text-gray-400 pointer-events-none transition-transform duration-200"
        ...
        <div v-show="isOpen"
            ...
            <div v-if="filteredOptions.length === 0" class="py-4 text-center text-gray-400 text-sm">
                暂无数据
            </div>

            <div v-else ref="dropdownRef" class="overflow-y-auto custom-scrollbar"
                :style="{ height: `${dropdownHeight}px` }" @scroll="handleScroll" @mousedown.prevent>
                <div :style="{ height: `${totalHeight}px`, position: 'relative' }">
                    <div :style="{ transform: `translateY(${offsetY}px)` }">
                        <div v-for="item in visibleOptions" :key="item.adcode"
                            class="px-3 flex items-center text-sm cursor-pointer transition-colors duration-150 truncate"
                            :style="{ height: `${itemHeight}px` }" :class="[
                            ...
                            <span v-if="modelValue === item.adcode" class="ml-auto text-blue-600">
