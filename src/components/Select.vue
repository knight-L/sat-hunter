<template>
  <div class="relative w-full" ref="containerRef">
    <div
      class="relative flex items-center w-full bg-white dark:bg-gray-800 border rounded-xl cursor-text transition-all duration-200"
      :class="[
        isOpen
      ...
      <input
        ...
        :disabled="disabled"
        class="w-full px-3 py-2 bg-transparent outline-none text-sm text-gray-700 dark:text-gray-200 placeholder-gray-400 dark:placeholder-gray-500"
        @input="handleInput"
        @focus="handleFocus"
        @blur="handleBlur" />

      <span
        class="absolute right-3 text-gray-400 dark:text-gray-500 pointer-events-none transition-transform duration-200"
    ...
    <div
      ...
      <div
        ...
        class="py-4 text-center text-gray-400 dark:text-gray-500 text-sm">
        暂无数据
      </div>
        ...
        <div :style="{ height: `${totalHeight}px`, position: 'relative' }">
          <div :style="{ transform: `translateY(${offsetY}px)` }">
            <div
              ...
              {{ item.name }}
              <span
                class="ml-2 text-[10px] text-gray-400 dark:text-gray-500 uppercase">
                {{ item.level }}
              </span>
              <span
